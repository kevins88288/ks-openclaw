...
describe("sendMessageMattermost", () => {
  ...
  it("DM channel name — bot first: resolves to real DM channel id", async () => {
    ...
    const result = await sendMessageMattermost(`channel:${BOT_ID}__${TARGET_USER_ID}`, "hello");

    expect(mockState.fetchMattermostMe).toHaveBeenCalledTimes(1);
    expect(mockState.createMattermostDirectChannel).toHaveBeenCalledWith(expect.anything(), [
      BOT_ID,
      TARGET_USER_ID,
    ]);
  ...
  it("DM channel name — bot second (reversed): resolves to real DM channel id", async () => {
    ...
    const result = await sendMessageMattermost(`channel:${TARGET_USER_ID}__${BOT_ID}`, "hello");

...
export function registerQueueCommands(
  ...
  const queue = program
    .command('queue')
    .description('Manage agent job queues');
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const queueNames = agentId 
          ? [`agent:${agentId}`]
          : await getQueueNames(connection);
      ...
      try {
        ...
        for (const queueName of queueNames) {
          ...
          for (const job of jobs) {
            ...
            console.log(`    Task: ${data.task.substring(0, 60)}${data.task.length > 60 ? '...' : ''}`);
...
async function getQueueNames(connection: Redis): Promise<string[]> {
  const keys = await connection.keys('bull:agent:*:meta');
  return keys.map(key => {

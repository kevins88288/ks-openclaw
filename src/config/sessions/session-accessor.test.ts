...
describe("session accessor file-backed seam", () => {
  ...
  it("commits reply initialization from fresh disk when the mutable writer cache diverges", async () => {
    const sessionKey = "agent:main:main";
    await upsertSessionEntry(
      { sessionKey, storePath },
      { sessionId: "first-session", updatedAt: 10 },
    );

    const snapshot = loadReplySessionInitializationSnapshot({ sessionKey, storePath });
    const cachedEntry = loadSessionStore(storePath, { clone: false })[sessionKey];
    if (!cachedEntry) {
      throw new Error("expected mutable session store cache");
    }
    // The CAS snapshot deliberately bypasses the cache and still sees
    // first-session. The writer would otherwise take this stale mutable object.
    cachedEntry.sessionId = "stale-cached-session";

    const committed = await commitReplySessionInitialization({
      activeSessionKey: sessionKey,
      agentId: "main",
      expectedRevision: snapshot.revision,
      sessionEntry: { sessionId: "next-session", updatedAt: 20 },
      sessionKey,
      storePath,
    });
    ...
    expect(committed).toMatchObject({
      ok: true,
      sessionEntry: { sessionId: "next-session" },
    });
  });
